...
namespace arayuz.Kullanıcı_Arayüzü
{
    public partial class RegularUser : Form
    {
        ...
        public void idcekme(string ad, string sifre)
        {
            ...
            while (sqlRd.Read())
            {
                String idgelen = Convert.ToString(sqlRd["kullaniciId"]); //MessageBox.Show(idgelen);
                this.id = idgelen;
            }
        }
        ...
        {
            this.Hide();
            Topten to = new Topten(); to.kontrol(0);
            to.Show();
        }

        private void buttonpop10_Click(object sender, EventArgs e)
        {
            this.Hide();
            PopTen pto = new PopTen(); pto.kontrol(0);

            pto.Show();
        }

        private void buttonclassic10_Click(object sender, EventArgs e)
        {
            this.Hide();
            ClassicTen ct = new ClassicTen(); ct.kontrol(0);

            ct.Show();
        }

        private void buttonjazz10_Click(object sender, EventArgs e)
        {
            this.Hide();
            JazzTen jt = new JazzTen(); jt.kontrol(0);
